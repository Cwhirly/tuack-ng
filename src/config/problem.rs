...
impl ProblemConfig {
    ...
    pub fn test_data(&self) -> Vec<FsTestData<'_>> {
        self.runtime
            .data
            .iter()
            .map(|item| FsTestData::from_data(self.path.join("data"), item))
            .collect()
    }

    /// 构造样例数据的 [`FsTestData`] 列表（从 `sample/` 读取）。
    pub fn sample_data(&self) -> Vec<FsTestData<'_>> {
        self.runtime
            .samples
            .iter()
            .map(|item| FsTestData::from_sample(self.path.join("sample"), item))
            .collect()
    }
}
